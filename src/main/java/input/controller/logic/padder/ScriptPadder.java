...
public class ScriptPadder implements Padder {
	...
	private static String getDefaultScript() throws IOException {
		return IOUtils.toString(
				ScriptPadder.class.getResourceAsStream("/defaultScript.js"),
				Constants.ENCODING);
	}
	...
	private ScriptEngine getEngine() throws ScriptException {
		if (engine == null) {
			engine = Null.nvl(factory.getEngineByName("rhino"),
					factory.getEngineByMimeType("text/javascript"));
			engine.put("console", LOG);
			engine.put("robot", getRobot());
			engine.eval("function " + FUNCTION_NAME
					+ "(event){console.debug('Function not implemented');}");
		}
		return engine;
	}
